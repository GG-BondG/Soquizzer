...
from app.entity import Answer, Question, QuestionType, Quiz, Section


def _in_scope(query: Select, section_id: str | None, course_id: str | None) -> Select:
    """Restrict a query on questions to one section, or to every section of one course."""
    query = query.join(Quiz, Quiz.id == Question.quiz_id)
    if section_id is not None:
        return query.where(Quiz.section_id == section_id)
    return query.join(Section, Section.id == Quiz.section_id).where(Section.course_id == course_id)
...
class AnswerRepository:
    ...
    def still_wrong(
        ...
        query = _in_scope(query, section_id, course_id)
    ...
    def stats_by_type(
        self, *, section_id: str | None = None, course_id: str | None = None
        ...
        query = _in_scope(query, section_id, course_id).group_by(Question.type).order_by(Question.type)
