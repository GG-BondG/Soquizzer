...
class Settings(BaseSettings):
    ...
    ocr_enabled: bool = True  # scanned PDFs (no text layer) are read by Gemini instead of failing
    ocr_pages_per_request: int = 10
    ocr_max_pages: int = 300
    max_material_chars: int = 400_000  # all of a section's PDF JSON goes into one quiz prompt
