...
@router.post("/api/sections/{section_id}/materials", status_code=201, response_model=MaterialResponse)
def create_material(section_id: str, file: UploadFile, service: MaterialService = Depends(get_material_service)):
    return service.create_from_pdf(section_id, file.filename or "", file.file)


@router.get("/api/sections/{section_id}/materials", response_model=list[MaterialResponse])
def list_materials(section_id: str, service: MaterialService = Depends(get_material_service)):
